...
public class ListEx {
	...
	public static void main(String[] args) {
		...
		for(int i = 1 ; i <= 5 ; i++) {
			list.add(i);// 정수 i가 박싱으로 Integer 클래스의 객체가 된 후 리스트에 추가
		}
		//1을 더추가
		list.add(1);
		System.out.println(list);
		//1을 제거
		list.remove((Integer)1);//첫번째 만나는 1을 제거하고 제거가 되어서 true를 반환
		System.out.println(list);
		//1번지에 있는 객체를 제거(3을 제거)
		Integer num = list.remove(1);//1번지에 있는 요소를 제거하고 제거한 요소를 반환
		System.out.println(num);
		System.out.println(list);
		System.out.println(list.size());//저장된 객체의 수
