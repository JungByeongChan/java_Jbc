package day17.practice.servise;
...
public class ShopServiceImp implements ShopService {
	...
	public int store(Product[] list, int count, Product product) {
		//입고할 제품이 없으면 입고하지 않고, 기존 제품 수를 알려줌.
		if(product == null) {
			return count;
		}
		//제품리스트가 없으면
	...
	private int indexOf(Product[] list, int count, Product product) {
		...
		for(int i = 0; i < count ; i++) {
			Product tmp = list[i];//제품 리스트에서 꺼낸 정보를 tmp에 저장
			if(tmp == null || tmp.getName() == null || product.getName() == null)
			{//제품리스트에 잇는 제품이 없거나 이름이 없거나 찾을 제품명이 없으면 못찾음
				return -1;
			}
	...
	@Override
	public void printProdunt(Product[] list, int count, String search) {
