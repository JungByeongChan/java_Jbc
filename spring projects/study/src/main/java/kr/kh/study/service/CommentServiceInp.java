...
@Service
public class CommentServiceInp implements CommentService{
	...
	public boolean updateComment(CommentVO comment, MemberVO user) {
		...
		if(comment == null || comment.getCo_num() == 0 || comment.getCo_contents() == null || comment.getCo_contents().trim().length() == 0) {
			return false;
		}
		// 덧글 존재 확인 및 작성자 확인
		CommentVO dbComment = commentDao.selectComment(comment.getCo_num());
