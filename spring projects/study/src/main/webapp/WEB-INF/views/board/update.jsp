...
<body>
	...
	<form action="<c:url value='/board/update'/>" method="post" enctype="multipart/form-data">
		...
		<c:forEach items="${fileList}" var="file">
			<div class="box">
				<a href="<c:url value='/download${file.fi_name}'/>" download="${file.fi_ori_name}">${file.fi_ori_name } </a><br>
	...
	<script type="text/javascript" src="//code.jquery.com/jqeury-3.6.1.js"></script>
	<script type="text/javascript">
		$('.btn-del').click(function(e){
			e.preventDefault();
			let fi_num = $(this).data('num');
			$('.btn-submint').before('<input type="file" name="files"><br>');
			$('.btn-submint').before('<input type="hidden" name="delNums" value="'+ fi_num +'"><br>');
			$(this).parent().remove();
		})
	</script>
</body>
