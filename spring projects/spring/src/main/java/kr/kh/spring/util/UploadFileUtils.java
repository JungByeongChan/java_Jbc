...
public class UploadFileUtils {
	...
	public static String uploadFile(String uploadPath, String originalFileName, byte[] fileData) throws IOException {
		...
		//같은 파일명을 가지는 파일을 업로드할 때 덮어쓰기가 되지 않게 처리
		UUID uuid = UUID.randomUUID();//범용고유식별자라고도 부름 
		String savedFileName = uuid.toString() + "_" + originalFileName; // 8-4-4-4-12(_파일명)
	...
	private static String uploadFileName(String uploadPath, String savedPath, String savedFileName) {
		...
		return fileName.replace(File.separator, "/");//파일이 문자열이기 때문에 "/"로 문자열처리를 해주어야함
	}
	...
	private static String calcPath(String uploadPath) {
		...
		// \\2023
		String yearPath = File.separator + cal.get(Calendar.YEAR);//separator 운영체제에 맞춰서 변형해주는 함수
		// \\2023\09
		String monthPath = yearPath + File.separator + new DecimalFormat("00").format(cal.get(Calendar.MONTH)+1);
		//deciamlformat 은 비어있는 자리를 채워주는것 이 경우 03월의처럼 0이 비어있는경우
