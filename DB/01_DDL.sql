...
*/
/*
DB 생성 : MYSQL 에서는 DB와 스키마가 같은 것으로 인식 / 개념이 같은 것은 아님
create schema DB명;
create database DB명;
drop schema DB명;
drop database DB명;
...
제약조건 옵션 : 
- 참조무결성을 지키기 위해서 참조되는 테이블의 기본키값이 변경되면 참조하는 테이블의 값을 어떻게 할 것인지를 결정하는 값
RESTRICTED : 참조되고 잇는 경우 기본키의 값을 바꾸지 못하게 함
- 수강에 등록된 과목들은 과목 코드를 변경할 수 없음
CASECADE : 참조되고 있으면 기본키의 값이 바꿀 때 참조하는 테이블의 값도 같이 바뀜
- 수강에 등록된 과목 코드가 변경되면 수강 테이블의 과목 코드도 변경
SET NULL : 참조되고 있으면 기본키의 값이 변하면 참조하는 테이블의 값이 NULL로 바뀜
- 단, 이 경우는 외래키가 NULL 허용인 경우만 가능
NO ACTION : 제약조건 옵션을 선택하지 않은 경우 자동으로 선택. RESTRICTED와 동일
SET DEFAULT : 참조되고 있으면 기본키의 값이 변할 때 참조하는 테이블의 값이 기본값으로 바뀜
*/
-- 대학생(학번, 이름, 학과)
...
-- 과목(과목코드, 과목명, 학점, 시수)
-- 과목코드 앞 3자리는 종류, 뒤 3자리는 수자 MS001
 ...
 /*
 데이터베이스는 수정할 수 없지만 테이블은 수정할 수 있음
 alter : 테이블을 수정
 - 속성 추가
 ...
 - 속성명 수정
 alter table 테이블명 change 기존속성명 새 속성명 타입;
 - 속성 삭제
...
-- 수강 테이블 기본키에 있는 auto_inrement 속성을 제거
...
alter table course add constraint FK_STUDENT_NUM
 foreign key(stduent_num) references student(num);
