...
as : 속성 또는 테이블명을 일시적으로 변경해서 사용할 때 사용하는 키워드
...
= : 같다를 의미 null과 비교는 불가능
!= : 같지않다를 의미 null과 비교는 불가능
<> : 같지않다를 의미 null과 비교는 불가능 // 
and : ~하고, 자바의 && 연산자
or : ~하거나, 자바의 || 연산자
is null : null인지 확인
is not null : null이 아닌지 확인
like : 와일드카드와 함께 사용되며, 값이 주어진 와이드가 포함된 문자열과 비교할 때 사용
_ : 한글자를 의미하는 와일드 카드
% : 0글자이상을 의미하는 와일드 카드
속성명 between A and B : 속성의 값이 A이상 B이하인 조건을 만족할 때 사용
=> 속성명 >= A and 속성명 <= B
속성명 in(값1, 값2, ..., 값n) : 곡성의 값이 값1이거나 값2이거나 ... 값n인 조건을 만족할 때 사용
...
order by 속성명1 정렬방법 [, 속성명2 정렬방법, ...] 반드시 하나는 와야하지만 뒤는 생략가능/[]는 생략가능하다는 표시
=> 속성명1을 정렬방법을 기준으로 정렬하고, 속성명1의 값이 같으면 속성명2를 정렬방법 기준으로 정렬...
- 정렬방법
...
select * from student where name like '___'; -- _를 3개로 이름이 3자라는걸 정의
...
-- 학점이 2하점 또는 3학점 이상인 과목의 수강 정보를 조회
...
-- 학생 정보를 이름순으로 정렬하여 조회 asc는 기본값
select * from student order by name;
-- 학생 정보를 내림차순으로 정렬하여 조회
...
-- 과목별 수강 학생수를 과목코드, 수강생 수로 조회하는 쿼리
