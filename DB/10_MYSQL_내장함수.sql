...
instr(기준문자열, 찾을문자열);
: 기준문자열에서 찾을 문자열이 몇번째에 있는지 알려주는 함수, 없으면 0. 1부터 시작

format(숫자, 소수점 자리수);
: 숫자를 소수점 자리까지 표현하고, 정수부분은 3개단위로 ,를 추가 해줌

right(문자열, 숫자)/left(문자열, 숫자);
: 문자열의 오른쪽/왼쪽에서 숫자개수만큼 문자열을 추출. 오른쪽/왼쪽을 기준으로 고정된 길이의 문자열을 추출할 때
...
from student;
select instr(title, '대학') as 대학위치, title from subject;
...
select adddate(now(), interval 1 day); -- 현재 시간을 기준으로 하루 다음 날짜
select adddate(now(), interval 1 month);
select adddate(now(), interval 1 year);
select subdate(now(), interval 1 day);
select addtime(now(), '1 0:0:0'); -- 현재 시간을 기준으로 하루 다음 날짜
select datediff('2023-08-02 13:00:00', '2023-08-03 14:00:00');
select timediff('2023-08-02 13:00:00', '2023-08-03 14:00:00');